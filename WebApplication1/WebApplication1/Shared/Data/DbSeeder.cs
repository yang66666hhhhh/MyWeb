...
public static class DbSeeder
{
    public static async Task SeedAsync(AppDbContext context, ILogger? logger = null)
    {
        ...
        if (!await context.WorkProjects.AnyAsync())
        {
            ...
            {
                new() { ProjectName = "生产线升级项目", ProjectCode = "PROJ001", ProjectType = WorkProjectType.RAndD, CustomerName = "内部", Description = "生产线设备升级改造", StartDate = today.AddDays(-60), EndDate = today.AddDays(30), Status = WorkProjectStatus.Active, Sort = 1, CreatedAt = now },
                new() { ProjectName = "质量改进项目", ProjectCode = "PROJ002", ProjectType = WorkProjectType.Support, CustomerName = "内部", Description = "提升产品质量和良品率", StartDate = today.AddDays(-45), EndDate = today.AddDays(45), Status = WorkProjectStatus.Active, Sort = 2, CreatedAt = now },
                new() { ProjectName = "设备维护项目", ProjectCode = "PROJ003", ProjectType = WorkProjectType.Support, CustomerName = "内部", Description = "设备定期维护保养", StartDate = today.AddDays(-30), EndDate = today.AddDays(60), Status = WorkProjectStatus.Active, Sort = 3, CreatedAt = now },
                new() { ProjectName = "新产品导入项目", ProjectCode = "PROJ004", ProjectType = WorkProjectType.RAndD, CustomerName = "客户A", Description = "新产品NPI导入", StartDate = today.AddDays(-20), EndDate = today.AddDays(90), Status = WorkProjectStatus.Active, Sort = 4, CreatedAt = now },
                new() { ProjectName = "能耗优化项目", ProjectCode = "PROJ005", ProjectType = WorkProjectType.Internal, CustomerName = "内部", Description = "降低生产能耗成本", StartDate = today.AddDays(-15), EndDate = today.AddDays(75), Status = WorkProjectStatus.Active, Sort = 5, CreatedAt = now },
                new() { ProjectName = "安全生产项目", ProjectCode = "PROJ006", ProjectType = WorkProjectType.Internal, CustomerName = "内部", Description = "安全生产管理提升", StartDate = today.AddDays(-10), EndDate = today.AddDays(100), Status = WorkProjectStatus.Active, Sort = 6, CreatedAt = now },
            };
