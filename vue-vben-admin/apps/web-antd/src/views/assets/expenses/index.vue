...
const handleSubmit = async () => {
  if (!formState.title || !formState.expenseDate || !formState.amount) {
...
<template>
  <Page description="记录和管理支出" title="支出管理">
    ...
    <Card title="支出记录">
      <template #extra>
        <Button type="primary" @click="handleAdd">记录支出</Button>
      </template>
      <Table
        :columns="columns"
        ...
        <template #bodyCell="{ column, record }">
          ...
          <template v-else-if="column.key === 'action'">
            <div class="flex gap-2">
              <Button type="link" size="small" @click="handleEdit(record as Expense)">编辑</Button>
